...
import {
  ...
  type AdoIntegrationInstance,
} from "@/lib/exchanges/ado/schema/config.schema";
...
export const SetupPat = ({ open, onOpenChange, onComplete }: SetupPatProps) => {
  ...
  const [connectionSuccessful, setConnectionSuccessful] = useState<
    boolean | null
  >(null);
  ...
  const isOrgOrBaseUrlValid =
    (organization ?? "").trim().length > 0 || (baseUrl ?? "").trim().length > 0;
  ...
  const testConnection = async (): Promise<{
    ...
    try {
      ...
      const fetchedUserId = await fetchAuthenticatedUserId(
        url,
        personalAccessToken
      );
  ...
  const handleConfirmAddInstance = async () => {
    try {
      ...
      if (!connectionResult.success || !connectionResult.userId) {
        console.error(
          "Connection test failed or userId not retrieved. Instance not saved."
        );
      ...
      if (onComplete) {
        onComplete(true);
        TrackEvent("ado_pat_instance_added", {
          defaultBaseUrl: baseUrl?.match(/^https:\/\/dev.azure.com\//),
        });
      }
      onOpenChange(false);
    } catch (e) {
