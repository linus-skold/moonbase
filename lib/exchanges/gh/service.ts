...
export const classifyItem = (item: TypedItem) => {
  const workItemClassifier = createWorkItemClassifier(GITHUB_WORK_ITEM_KIND_MAPPING);
  workItemClassifier.classify({title: item.title, labels: [ item.itemStatus ], typeName: "issue" });
}

export const fetchInboxItems = () : Promise<TypedItem[]> => {
  
  // return service.fetchAllInboxItems();
}
