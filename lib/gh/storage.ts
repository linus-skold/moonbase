// Storage utilities for GitHub configuration

import type { GhConfig } from './schema/config.schema';

const STORAGE_KEY = 'gh-config';
...
export function loadConfig(): GhConfig | null {
  ...
  try {
    ...
    if (data.version !== STORAGE_VERSION) {
      console.warn('GitHub configuration version mismatch, migration may be needed');
    }
 ...
 */
export function saveConfig(config: GhConfig): boolean {
  if (typeof window === 'undefined') {
    return false;
  }
 ...
 */
export function exportConfig(config: GhConfig): void {
  // Remove sensitive data before export
  const sanitized: GhConfig = {
    ...config,
  ...
  link.href = url;
  link.download = `gh-config-${new Date().toISOString().split('T')[0]}.json`;
 ...
 */
export async function importConfig(file: File): Promise<GhConfig | null> {
  try {
